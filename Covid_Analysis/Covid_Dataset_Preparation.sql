...
select  sum(total_cases) as 'Total Cases',sum(cast(new_deaths as int)) as 'Total Deaths',
...
-- group by date
 ...
 select location,population,max(total_cases) as heighestinfectioncount,max((total_cases/population))*100  as
 percentagepopulationinfected
 from PortfolioProjects..['owid-covid-data$']
 ...
 --4
  ...
  select location,population,date,max(total_cases) as heighestinfectioncount,max((total_cases/population))*100  as
 percentagepopulationinfected
 from PortfolioProjects..['owid-covid-data$']
